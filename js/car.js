class Carro
{
	...
	Update(delta) {
		...
		//Acceleration
			...
			if (this.velocity.x != 0 && Math.abs(this.throttle) != 1 && Math.abs(this.velocity.x) < 0.0005) {
				this.velocity.x = 0;
				this.throttle = 0;
			}
	...
	ApplyVelocity() {
		//this.car.position += this.velocity;
	}
	...
	OnUnleft() {
		...
		this.turn = this.brakePressed && this.turn != 1 ? 1 : this.turn;
	}
